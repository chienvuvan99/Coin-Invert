...
export const DraggableGrid = function <DataType extends IBaseItemType>(
  ...
  const blockPositions = useRef<IPositionOffset[]>([]).current;
  const orderMap = useRef<IMap<IOrderMapItem>>({}).current;
  const itemMap = useRef<IMap<DataType>>({}).current;
  const items = useRef<IItem<DataType>[]>([]).current;
  const [blockHeight, setBlockHeight] = useState(0);
  const [blockWidth, setBlockWidth] = useState(0);
  // const gridHeight = useRef<Animated.Value>(new Animated.Value(0)).current;
  const [hadInitBlockSize, setHadInitBlockSize] = useState(false);
  const dragStartAnimatedValue = useRef(new Animated.Value(1)).current;
  ...
  const rowItem = ({item, index}: any) => {
    return (
      <Block
        onPress={onBlockPress.bind(null, index)}
        onLongPress={setActiveBlock.bind(null, index, item.itemData)}
        panHandlers={panResponder.panHandlers}
        style={getBlockStyle(index)}
        dragStartAnimationStyle={getDragStartAnimation(index)}
        delayLongPress={props.delayLongPress || 300}
        key={item.key}
        renderItem={props.renderItem(item.itemData, orderMap[item.key].order)}
      />
    );
  };
  ...
  return (
    <View style={[styles.draggableGrid, props.style]} onLayout={assessGridSize}>
      {hadInitBlockSize && renderList()}
    </View>
  );
};
